            ...
            $.ajax({
                    ...
                    success:function (data) {
                        if (data.success) {
                            ...
                            html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${id.name}</b> <small style="color: gray;" id = "s'+data.ar.id+'"> '+data.ar.createTime+' 由'+data.ar.createBy+'</small>'
		...
		$.ajax({
					...
					success:function (data) {
						...
						$.each(data,function (i, n){
							...
							html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${id.name}</b> <small style="color: gray;" id= "s'+n.id+'"> '+(n.editFlag == 0?n.createTime:n.editTime)+' 由'+(n.editFlag == 0?n.createBy:n.editBy)+'</small>'
							html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">'
    ...
    function deleteRemark(id) {
        $.ajax({
                ...
                success:function (data) {
                    ...
                    if (confirm("确定删除吗？")) {
                        if (data.success) {
                            //这样不行， 使用了before， 每一次删除之后都会保留原有的数据
                            $("#"+id).remove();
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>市场活动-${id.name} <small>${id.startDate} ~ ${id.endDate}</small></h3>
		</div>
